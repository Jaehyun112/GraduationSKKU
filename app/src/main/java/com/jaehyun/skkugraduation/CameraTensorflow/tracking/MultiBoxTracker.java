...
package com.jaehyun.skkugraduation.CameraTensorflow.tracking;
...
import com.jaehyun.skkugraduation.CameraTensorflow.CameraActivity;
import com.jaehyun.skkugraduation.CameraTensorflow.Classifier.Recognition;
...
import com.jaehyun.skkugraduation.CameraTensorflow.env.BorderedText;
...
public class MultiBoxTracker {
  ...
  public static OnDetectedListner onDetectedListner;

  public interface OnDetectedListner{
    public void onDtected(RectF rect);
  }

  public static void setOnDetectedListner(OnDetectedListner detectedListner){
    onDetectedListner = detectedListner;
  }
  ...
  public void draw(final Canvas canvas) {
    Paint boxFillPaint = new Paint();
    boxFillPaint.setColor(Color.parseColor("#0080ff"));
    boxFillPaint.setStyle(Style.FILL);
    boxFillPaint.setStrokeCap(Cap.ROUND);
    boxFillPaint.setStrokeJoin(Join.ROUND);
    boxFillPaint.setAlpha(100);

    Paint boxStrokePaint = new Paint();
    boxStrokePaint.setColor(Color.parseColor("#0080ff"));
    boxStrokePaint.setStyle(Style.STROKE);
    boxStrokePaint.setStrokeWidth(6.0f);
    boxStrokePaint.setStrokeCap(Cap.ROUND);
    boxStrokePaint.setStrokeJoin(Join.ROUND);
    boxStrokePaint.setStrokeMiter(100);
    ...
    for (final TrackedRecognition recognition : trackedObjects) {
      ...
      if(GuideItem.captureLike == GuideItem.CAPTURE_AUTO){
        canvas.drawRoundRect(trackedPos, cornerSize, cornerSize, boxFillPaint);
        canvas.drawRoundRect(trackedPos, cornerSize, cornerSize, boxStrokePaint);
      }
      ((Activity)context).runOnUiThread(new Runnable() {
        @Override
        public void run() {
          CameraActivity.notAcTextView.setVisibility(View.GONE);
        }
      });

      //TODO
      // set interface for listen
      if(trackedPos!=null){
        if(onDetectedListner!=null){
          onDetectedListner.onDtected(trackedPos);
        }
      }
      DetectorActivity.setOnDetectedListner(new DetectorActivity.OnNotAccurateListner() {
        @Override
        public void onNotAccureate() {
          ((Activity)context).runOnUiThread(new Runnable() {
            @Override
            public void run() {
              if(GuideItem.captureLike == GuideItem.CAPTURE_AUTO){
                CameraActivity.notAcTextView.setVisibility(View.VISIBLE);
              }
            }
          });
  ...
  public synchronized void onFrame(
    ...
    if (objectTracker == null && !initialized) {
      ...
      if (objectTracker == null) {
        //String message = "Object tracking support not found. " + "See tensorflow/examples/android/README.md for details.";
